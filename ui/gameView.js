...
import { SPRITE_ROWS, SPRITE_SIZE, SHEET_WIDTH, SHEET_HEIGHT } from "./constants.js";
import { isSolidCell, isDestructibleCell, isIndestructibleCell } from "./collision.js";
import { tileIndexForCell, imgStyleForIndex, ensureTilesetInfo } from "./tiles.js";
...
export function GameView(options) {
  ...
  const displayedCell = Math.max(1, Math.round(cellSize * (typeof mapScale === "number" ? mapScale : 1)));
  ...
  for (let y = 0; y < rows; y++) {
    for (let x = 0; x < cols; x++) {
      ...
      const imgStyle = imgStyleForIndex(idx, tilesetUrl, displayedCell, tileSrcSize, tileSpacing, tilesPerRowOpt, SHEET_WIDTH, SHEET_HEIGHT);
      const children = [{ tag: "img", attrs: { src: tilesetUrl, style: imgStyle, draggable: "false", alt: "" } }];
      if (showCollisionOverlays) {
        if (isIndestructible) {
          children.push({ tag: "div", attrs: { style: `position:absolute;left:0;top:0;width:100%;height:100%;background: rgba(255,0,0,0.28);z-index:12;pointer-events:none;` } });
        } else if (isBlock) {
          children.push({ tag: "div", attrs: { style: `position:absolute;left:0;top:0;width:100%;height:100%;background: rgba(0,200,0,0.28);z-index:12;pointer-events:none;` } });
        }
      }
  ...
  const playerNodes = playersWithPos.map((p) => {
    ...
    const offsetY = (SPRITE_ROWS && SPRITE_ROWS[colorIdx] && SPRITE_ROWS[colorIdx].offsetY) || 0;
    const margin = 4;
    const spacing = 1;
    const frame = 0;
    ...
    if (debugCollision) {
      ...
      const tileCell = (grid[ty] && grid[ty][tx]) !== undefined ? grid[ty][tx] : null;
      const collision = isSolidCell(tileCell);
      if (collision) {
        const colLeft = Math.round(tx * displayedCell);
        const colTop = Math.round(ty * displayedCell);
        collisionNodes.push({ tag: "div", attrs: { style: `position:absolute; left:${colLeft}px; top:${colTop}px; width:${displayedCell}px; height:${displayedCell}px; background: rgba(255,0,0,0.18); z-index:55; pointer-events:none;` } });
      }
      const hitW = 16;
      const hitH = 16;
      const hitLeft = Math.round((targetPx - hitW) / 2);
      const hitTop = Math.round((targetPx - hitH) / 2);
      collisionNodes.push({ tag: "div", attrs: { style: `position:absolute; left:${hitLeft}px; top:${hitTop}px; width:${hitW}px; height:${hitH}px; background: rgba(0,128,255,0.28); border: 1px solid rgba(0,128,255,0.6); z-index:65; pointer-events:none;` } });
    }
    return { tag: "div", attrs: { style: wrapperStyle, "data-player-id": p.id || "" }, children: [{ tag: "img", attrs: { src: playerSpriteUrl, style: innerStyle, draggable: "false", alt: "" } }, ...collisionNodes] };
  });
  ...
  return { tag: "div", attrs: { style: wrapperStyle, id: "game-map-root" }, children: [...tileNodes, ...playerNodes] };
}